...
Walk-forward realized-volatility forecasting for US equities. The target is one-day-ahead log realized volatility on a static S&P 500 universe. Models: naive, HAR-RV, GARCH(1,1), LightGBM, plus an optional LSTM. Evaluation: QLIKE, out-of-sample R² versus HAR, and pairwise Diebold–Mariano tests, with the last two years held out for a one-shot final test.
...
If you want the LSTM step on CUDA, install `torch` from the PyTorch CUDA index *before* `make install` so the editable install picks up the GPU build.
...
make lstm        # optional, requires torch
